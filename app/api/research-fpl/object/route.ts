...
export async function GET(request: Request) {
  ...
  const object = searchParams.get('object') || 'B_LEGAL_BEST_XI'
  const upstreamUrl = `${upstreamBase()}/api/v1/research-fpl/gameweek/${gw}/object?model=${encodeURIComponent(model)}&object=${encodeURIComponent(object)}`
